...
Point2D Cat::Move(World* world)
{
	...
	///
	///creating two queues: one for the points that have already been visited by the algorithm
	///and one for points that need to be checked
	/// 
	...
	while (!queue.empty())
	{
		...
		///the algorithm is checking, then erase it so that the next time the loop is run
		///the next point is used as the starting point, then add the current point
		///to the queue of visited points
		/// 
		...
		///
		///checking to see if the next point is out of range, and marking the current point as an exit if it is
		/// 

		if (abs(head.position.x) >= (world->getWorldSideSize() / 2) || abs(head.position.y) >= (world->getWorldSideSize() / 2))
		{
			exit = head;
			break;
		}
		...
		for (auto neigh : World::neighbors(head.position))
		{
			...
			///
			///under the correct conditions, add the point to queue, add 1 to 'weight' 
			///(weight counts how many spaces are in the current queue),
			///and mark the neighbor it came from as the current head
			/// 
	...
	///
	///cycles backwards through the visited points of the final queue to find the next point the cat should move to
	/// 
